...
/// </summary>
type Condition =
    | Binary of column: string * operator: ComparisonOperator * value: obj option
...
module Condition =
    ...
    let binary column operator value = Binary(column, operator, Some value)
    ...
    let isNull column = IsNull column
    ...
    let isNotNull column = IsNotNull column
    ...
    let and' cond1 cond2 = And(cond1, cond2)
    ...
    let or' cond1 cond2 = Or(cond1, cond2)
    ...
    let all conditions =
        ...
        | [ cond ] -> cond
        | head :: tail -> tail |> List.fold (fun acc cond -> And(acc, cond)) head
    ...
    let any conditions =
        ...
        | [] -> failwith "At least one condition required"
        | [ cond ] -> cond
        | head :: tail -> tail |> List.fold (fun acc cond -> Or(acc, cond)) head
...
module Query =
    ...
    let newParamName (state: QueryBuilderState) =
        ...
        name,
        { state with
            ParameterCounter = state.ParameterCounter + 1 }
    ...
    let addParam name value (state: QueryBuilderState) =
        { state with
            Parameters = (name, value) :: state.Parameters }
    ...
    let columns columnNames state =
        ...
        | Select query ->
            { state with
                QueryType = Select { query with Columns = columnNames } }
    ...
    let where condition state =
        ...
        | Select query ->
            { state with
                QueryType = Select { query with Where = Some condition } }
        | Update query ->
            { state with
                QueryType = Update { query with Where = Some condition } }
        | Delete query ->
            { state with
                QueryType = Delete { query with Where = Some condition } }
        ...
        | Select query ->
            { state with
                QueryType =
                    Select
                        { query with
                            OrderBy = (column, direction) :: query.OrderBy } }
    ...
    let limit n state =
        ...
        | Select query ->
            { state with
                QueryType = Select { query with Limit = Some n } }
    ...
    let offset n state =
        ...
        | Select query ->
            { state with
                QueryType = Select { query with Offset = Some n } }
    ...
    let values (columnValues: (string * obj) list) state =
        ...
        | Insert query ->
            { state with
                QueryType = Insert { query with Columns = columnValues } }
    ...
    let returning columns state =
        ...
        | Insert query ->
            { state with
                QueryType = Insert { query with Returning = Some columns } }
    ...
    let set (columnValues: (string * obj) list) state =
        ...
        | Update query ->
            { state with
                QueryType = Update { query with Set = columnValues } }
    ...
    let delete table =
        { QueryType = Delete { Table = table; Where = None }
          Parameters = []
          ParameterCounter = 0 }
...
module SqlGenerator =
    ...
    let rec private conditionToSql condition (state: QueryBuilderState) =
        ...
        | Binary(column, op, None) -> sprintf "%s %s NULL" column (operatorToString op), state
        | And(cond1, cond2) ->
    ...
    let private generateInsert (query: InsertQuery) (state: QueryBuilderState) =
        ...
        let paramNames =
            query.Columns |> List.mapi (fun i _ -> sprintf "@p%d" i) |> String.concat ", "

        let baseSql =
            sprintf "INSERT INTO %s (%s) VALUES (%s)" query.Table columns paramNames
        ...
        let parameters =
            query.Columns
            |> List.mapi (fun i (_, value) -> NpgsqlParameter(sprintf "p%d" i, value))
    ...
    let private generateUpdate (query: UpdateQuery) (state: QueryBuilderState) =
        ...
        let (setClause, stateWithSet) =
            query.Set
            |> List.fold
                (fun (clauses, accState) (col, value) ->
    ...
    let private generateDelete (query: DeleteQuery) (state: QueryBuilderState) =
        ...
        let parameters =
            stateWithWhere.Parameters
            |> List.rev
            |> List.map (fun (name, value) -> NpgsqlParameter(name, value))
    ...
    let private getValueFromOption (value: obj) : obj =
        if value = null then
            ...
            if isOptionType typ then
                ...
                if case.Name = "Some" then fields.[0] else null
            else
                value
