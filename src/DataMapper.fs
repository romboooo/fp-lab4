                    ...
                    | Some columnIndex ->
                        if reader.IsDBNull(columnIndex) then
                            if
                                ...
                                && prop.PropertyType.GetGenericTypeDefinition() = typedefof<option<_>>
                            then
                                let innerType = prop.PropertyType.GetGenericArguments().[0]
                                OptionConverter.createOption innerType null
                            else if prop.PropertyType.IsValueType then
                            ...
                            let convertedValue =
                                ...
                                then
                                    ...
                                    OptionConverter.createOption
                                        innerType
                                        (TypeConverter.convertValue innerType dbValue)
                                else
            ...
            if not includePrimaryKey && isPrimaryKey then
                ...
                let dbValue =
                    ...
                    then
                        ...
                        if case.Name = "Some" then fields.[0] else box DBNull.Value
                    else
                        value

                let columnName =
                    System.Text.RegularExpressions.Regex.Replace(prop.Name, "(?<=.)([A-Z])", "_$1").ToLowerInvariant()
